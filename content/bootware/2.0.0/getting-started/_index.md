...
In this Getting Started guide we describe how to bring up a common use case for Bootware - A/B partitioning for fallback and recovery. With Bootware 2.0 Beta, all boot artifacts are now contained in boot.img files, signed and verified. 
...
For Bootware 2.0.0-Beta, a confirmation is required that you are aware that you are installing Beta code that is not intended for production. The install will identify your Pi and OS and then prompt you if you'd like to include hardware signing. The HSM60, ZYMKEY5 and HSM6 support hardware signing. All Zymbit products support software signing. For the purpose of this tutorial, we will use software signing. Use the arrow keys to move the selection to `> No`.
...
Selecting will open a sub-menu with the following options:

```
❯ Turn on Signed Boot (Flash EEPROM)
  Configure the boot signing key
  Re-sign boot images with the configured boot signing key
  Exit
```

- `Turn on Signed Boot (Flash EEPROM)` - Enables the Signed Boot flag, and reflashes the EEPROM. The Boot Signing Key (BSK) configured will be used. Bootware uses a unique key by default that cannot be retrieved. Therefore it is highly recommended that when turning on Signed Boot, you also choose the `Configure the boot signing key` with your own private BSK at the same time. Once Signed Boot is turned on, the Pi will only be able to start up from a FAT disk image file signed with your (private) Boot Signing Key (BSK), so do not lose it! If you wish to disable this behavior, the Pi can still be restored to factory settings with the [rpiboot utility](https://github.com/raspberrypi/usbboot), or via the Pi Imager Misc utility Bootloader utility.

- `Configure the boot signing key` - Imports the private BSK PEM. Supply the path to your private key file in PEM format. This will trigger a re-sign of all existing boot disk image artifacts in /boot/firmware, emitting new signature files (.sig) generated by the new BSK.

- `Re-sign boot images with the configured boot signing key` - Included for development purposes. Triggers a re-sign with the configured boot signing key. Normal Bootware actions such as updates, rollback, or recovery will re-sign automatically.
...
While the Bootware update process takes place, progress messages will display on the console.
...
Bootware 2.0.0 BETA introduces support for secure boot integration of signed boot images. At this point, Bootware has configured your system to always use signed boot.img files for booting into zboot, or to your A or B partition. To continue on and use your own private BSK, enforce SIGNED_BOOT images, and configure Secure Boot, follow the steps here: [Secure Boot](../secure-boot).
