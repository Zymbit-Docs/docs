...
The `file_manifest` is a file that keeps track of the files that are specified by the user to wrap into an overlay image using `zbcli imager`. `file_manifest` can contain both folder paths and file paths.
...
Example: have `zbcli manifest` add the kernel to the `file_manifest`
