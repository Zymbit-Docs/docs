...
An ECDSA (Elliptic Curve Digital Signature Algorithm) signature is generated by a private key in the elliptic curve family. We use
these signatures as proof of identity (private key owner) and proof that the message has not been tampered with during the journey.

Signatures are effectively a user's wax seal on an envelope. Signatures play a huge role at the low level in blockchain. All data
must be signed before being transmitted to the blockchain.
